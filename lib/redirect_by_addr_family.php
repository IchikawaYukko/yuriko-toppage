<?php
//Make different redirects by accessing address family
...
function is_same_origin($location) {
    // Check redirect target includes : of 'http://'
    if (strpos($location, ':')){
        return false;
    }
    return true;
...
if (!strlen($loc_ipv4) || !strlen($loc_ipv6)) {
    ...
    echo 'missing ipv4= or ipv6= URL parameter!';
    exit;
}
if (!is_same_origin($loc_ipv4) || !is_same_origin($loc_ipv6)) {
    header($_SERVER["SERVER_PROTOCOL"].' 404', TRUE, 404);
    
    echo 'Not same origin target on ipv4= or ipv6= URL parameter.';
    exit;
}
